...
public abstract class AbstractFluidActionIterable2<T> {
    World world;
     ...
     */
    protected AbstractFluidActionIterable2(World world, BlockPos startPos, int maxRange) {
        this.world = world;
        this.startPos = startPos;
        this.maxRange = maxRange;
        // check cache or recompute
        FluidFlowCache flowCache = FFluidStatic.getFlowCache(world);
        ...
        if (cacheHitAndData.first) { // cache hit => use cached flow
            ...
            FlowTracer flowTracer = new FlowTracer(world, startPos, WPOConfig.COMMON.getMaxDist());
            Tuple2<Graph<BlockPos>, Set<BlockPos>> flowGraphAndBorder = flowTracer.getFlowGraphAndBorder();
