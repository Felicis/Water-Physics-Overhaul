...
public class FlowTracer {
    ...
    protected FlowTracer(World world, BlockPos startPos, int maxRange) {
        this.world = world;
    ...
    private List<Direction> getNextDirections() {
        return FFluidStatic.getDirsDownRandomHorizontal(world.getRandom()); // fluid does not flow UP
    }

    public Tuple2<Graph<BlockPos>, Set<BlockPos>> getFlowGraphAndBorder() {
                ...
                if (range < maxRange) { // if not max range reached  => generate new pos's
                    ...
                    for (Direction nextDir : getNextDirections()) {
                        ...
                        if (!allVisited.contains(posNew)) { // do not visit twice
                            ...
                            if (FFluidStatic.canFlowAndHold(world, posCurrent, nextDir)) {
                                // add node to graph
