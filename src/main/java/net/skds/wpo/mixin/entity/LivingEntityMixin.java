...
public abstract class LivingEntityMixin extends Entity {
    ...
    @Shadow protected abstract float getFrictionInfluencedSpeed(float pFriction);

    @Shadow protected abstract float getWaterSlowDown();

    public LivingEntityMixin(EntityType<?> pType, World pLevel) {
        super(pType, pLevel);
        WPO.LOGGER.error("LivingEntityMixin constructor should never be called (IDE forced me to implement it");
    }
    ...
    final int AMOUNT_FULL = 8; // before 1.17 it is not defined in FluidState
    ...
    private float correctWaterSpeedForDepth(float oldWaterSpeed) {
        if (oldWaterSpeed == 0.02F) {  // this is the init value: this only checks if the call is right after init
            ...
            float waterSpeed = landSpeed - (landSpeed - oldWaterSpeed) * fluidstate.getAmount() / AMOUNT_FULL;
...
//                    ", newSpeed=" + waterSpeed + ", blockFriction=" + f3 + ", landSpeed=" + landSpeed);
    ...
    private float correctWaterDragForDepth(float oldWaterDrag) {
        ...
        if (this.isSprinting() && oldWaterDrag == 0.9F || !this.isSprinting() && oldWaterDrag == this.getWaterSlowDown()) {
            ...
            float waterDrag = landDrag + (oldWaterDrag - landDrag) * fluidstate.getAmount() / AMOUNT_FULL;
